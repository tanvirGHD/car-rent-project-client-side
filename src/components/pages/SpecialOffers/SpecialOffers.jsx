            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                {offers.map((offer) => (
                    ...
                    >
                        ...
                        <button className="mt-4 text-blue-500 hover:underline transition">Learn More</button>
                    </div>
                ))}
            </div>
            ...
            {selectedOffer && (
                <div className="fixed inset-0 bg-gray-800 bg-opacity-50 flex justify-center items-center z-50">
                    <div className="bg-white p-8 rounded-lg shadow-lg w-96 max-w-lg">
                        <h2 className="text-2xl font-bold text-gray-800 mb-4">{selectedOffer.title}</h2>
                        <p className="text-gray-700 mb-4">{selectedOffer.description}</p>
                        <span className="text-lg font-bold text-gray-900">{selectedOffer.price}</span>
