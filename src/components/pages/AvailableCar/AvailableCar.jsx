            ...
            <div className={view === "grid" ? "grid md:grid-cols-2 lg:grid-cols-3 gap-6" : "space-y-6"}>
                {filteredCars.map(car => (
                    ...
                    >
                        ...
                        />
                        <h3 className="text-xl text-[#00C1A2] mb-2 transition-all duration-500 ease-in-out">{car.model}</h3>
                        <p><strong>Price:</strong> {car.dailyRentalPrice} BDT/day</p>
                        <p><strong>Features:</strong> {car.features}</p>
                        <p><strong>Description:</strong> {car.description}</p>
                        <p><strong>Location:</strong> {car.additionalInfo}</p>
                        <p><strong>Booking Count:</strong> {car.bookingCount}</p>
                        <button 
                            className="bg-[#00C1A2] text-white py-2 px-4 rounded mt-4 w-full transition-all duration-500 ease-in-out transform hover:scale-105"
                            onClick={() => window.location.href = `/car-details/${car._id}`}
                        >
                            Book Now
                        </button>
                    </div>
